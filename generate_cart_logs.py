    ...
    for day in range(days):
        ...
        for _ in range(num_logs_per_day):
            ...
            if actionType == "ADD" and random.random() < 0.3:
                ...
                remove_log["actionType"] = "REMOVE"
                actions.append(remove_log)
                cart_state[user_id].discard(product["name"])

    # MongoDB에 넣기
    if actions:
        cart_collection.insert_many(actions)

    print(f"{len(actions)}개의 장바구니 로그가 MongoDB에 저장되었습니다!")
